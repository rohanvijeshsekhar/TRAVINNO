...
// Original SVG paths from the first version of the Travinno logo
...
// Combine both paths into a single continuous outline path
// This connects the inner T and outer pin, starting at one side of the bottom gap
// and ending at the other side of the bottom gap.
// This naturally eliminates the unwanted line across the bottom gap!
const combinedPath = innerTPath + " L " + outerPinPath.substring(13);
  ...
  return (
    ...
    >
      <div className="loader-logo-wrapper" style={{ position: 'relative', width: '120px', height: '156px' }}>
        ...
        >
          {/* Step 1 & 2: Single continuous hand-drawn stroke animation (Duration 1.8s, fades out at 1.8s) */}
          <motion.path
            d={combinedPath}
            fill="none"
            stroke="#FFFFFF"
            ...
            transition={{
              pathLength: { duration: 1.8, ease: [0.25, 0.1, 0.25, 1.0] },
          ...
          <motion.path
            d={combinedPath}
            fillRule="evenodd"
            fill="#EA1C29" // Original brand red
            stroke="none"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            transition={{
